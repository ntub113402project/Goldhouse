    ...
    try:
        ...
        cursor.execute("""
            SELECT title, city, district, address, type, agency, layer, pattern, 
                   price, deposit, content, size, hid , cluster, phone
        ...
        similarhouses = results[13]  
        cursor.execute("""
            SELECT hid, title, city, district, size, price, pattern
            FROM new_housedetail 
            WHERE cluster=%s AND hid!=%s
        """, (similarhouses, hid))
        similarhouses = cursor.fetchall()

        similarhouseslist = []
        for house in similarhouses:

            PHOTO_DIRECTORY = 'C:/jpg'
            similar_house_photo_directory = os.path.join(PHOTO_DIRECTORY, house[0])
            if not os.path.exists(similar_house_photo_directory):
                similar_image_urls = []
            else:
                
                similar_image_urls = f"http://4.227.176.245:5000/houses/{house[0]}/image1.jpg" 

            similarhouseslist.append({
                'hid': house[0],
                'title': house[1],
                'price': house[5],
                'city': house[2],
        ...
        if '先生' in agency_name:
            agency_name = agency_name[agency_name.find('先生') - 1] + '先生'
        elif '小姐' in agency_name:
            agency_name = agency_name[agency_name.find('小姐') - 1] + '小姐'
        else:
            agency_name = results[5]
        ...
        data = {
            ...
            'phone': results[14],
            'imageUrl': image_urls,
            'lessorname': agency_name,
            'similarhouses': similarhouseslist,
...
def manage_subscription():
    ...
    # 如果沒有 subscription_id，但有 member_id，則執行新增操作
        ...
        return jsonify({'message': 'Member ID or Subscription ID is required'}), 400
