...
def get_house_details(hid):
    cursor = db_ghdetail.cursor()
    cursor.execute("USE ghdetail")
    cursor.execute("SELECT title, address, type, agency, layer, pattern, price, deposit, content, size, address, hid FROM new_housedetail WHERE hid=%s", (hid,))
    ...
    agency_name = results[3].split(':')[-1] if ':' in results[3] else 'Unknown'
...
def search_properties():
    ...
    if filters.get('areas'):  # 地區
        if filters['areas'] and filters['areas'] != ['不限']:
            area_conditions = []
            for area in filters['areas']:
                area_conditions.append("address LIKE %s")
                params.append(f"%{area}%")
            query += " AND (" + " OR ".join(area_conditions) + ")"

    if filters.get('type') and filters['type'] != ['不限']:  # 房屋類型
        type_conditions = []
        for house_type in filters['type']:
            type_conditions.append("pattern = %s")
            params.append(house_type)
        query += " AND (" + " OR ".join(type_conditions) + ")"

    if filters.get('rentalrange') and filters['rentalrange'] != '不限':  # 租金範圍
    ...
    if filters.get('types') and filters['types'] != ['不限']:  # 房型
        type_conditions = []
        for house_type in filters['types']:
...
def add_subscription():
    ...
    cursor = db_gh_members.cursor()
    query = """
        INSERT INTO subscriptions (member_id, criteria, subscription_time) 
        VALUES (%s, %s, %s)
    """
    params = (memberid, json.dumps(criteria), datetime.datetime.now())
